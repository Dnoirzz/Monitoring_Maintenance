...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      final needsAppSelection = await ref
  ...
  Future<void> _showAppSelectionDialog() async {
    ...
    final mtApps = authState.mtApps;

    if (mtApps.isEmpty) {
      _showError('Tidak ada aplikasi MT yang tersedia');
      return;
    }

    final selectedApp = await showDialog<AvailableApp>(
      context: context,
          ...
          (context) => AlertDialog(
            ...
            content: SizedBox(
              ...
              child: ListView.builder(
                shrinkWrap: true,
                itemCount: mtApps.length,
                itemBuilder: (context, index) {
                  final app = mtApps[index];
                  return ListTile(
                    leading: const Icon(
                      Icons.engineering,
                      color: Color(0xFF0A9C5D),
                    ),
                    title: Text(app.namaAplikasi),
                    subtitle: Text('Role: ${app.role}'),
    ...
    if (selectedApp != null && mounted) {
      try {
        await ref.read(authProvider.notifier).selectApp(selectedApp.aplikasiId);
        if (mounted) {
          _navigateToDashboard();
        }
      } on AuthException catch (e) {
